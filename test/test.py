...
class FlashRun:
    ...
    def check(self, regs=None):
        ...
        assert self.psram.transactions == self.model.bus_transactions, \
            "PSRAM transactions: dut %d model %d" % (self.psram.transactions, self.model.bus_transactions)
...
async def test_stream_sequence(dut):
    ...
    run.check()
    model_loads_stores = run.model.bus_transactions
    ...
    assert run.flash.transactions == 1 + model_loads_stores, "the stream restarts once per data access"
    assert run.flash.bytes_out >= 4 * (15 + 63), "the first stream delivered %d bytes" % run.flash.bytes_out
...
async def test_restart_on_branch(dut):
    ...
    run.check()
    taken = [d for d in run.model.trace if d["cls"] == "branch" and d["valid"] and d["pc"] != d["pc"]]
    # the branches taken before the epilogue: B a, BL sub, MOV pc lr (r15 write), B end = 4 restarts
    firsts = run.flash.first_addresses
    dut._log.info("flash first addresses: %s" % ["0x%04X" % a for a in firsts])
    assert firsts[:5] == [0x0000, 0x0010, 0x0030, 0x0024, 0x0038], firsts[:5]
...
async def test_data_access_interleave(dut):
    ...
    run = await FlashRun(dut, src).run()
    run.check()
    assert run.psram.transactions == run.model.bus_transactions
...
async def test_backpressure_hold(dut):
    ...
    # after release: exactly one accept of the parked word, then the address advances once
    first_load_seen = False
    for _ in range(4):
        await mid_cycle(dut)
        if int(c.if_id_load_out.value) and not first_load_seen:
            first_load_seen = True
            await mid_cycle(dut)
            assert int(d.fetch_addr.value) == addr_at_force + 4
            break
    assert first_load_seen, "the parked word was not accepted after the stall"
    layout = asm.EPILOGUE_LAYOUT
...
async def test_tcem_and_idle_pins(dut):
    ...
    assert run.psram.transactions == run.model.bus_transactions == 120
