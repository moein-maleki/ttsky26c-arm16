...
module bug_regression_testbench;
    ...
    initial begin
        ...
        force processor_unit.mem_out_stage_busy      = 1'b1;
        force processor_unit.exe_out_branch_taken    = 1'b1;
        force processor_unit.exe_out_branch_address  = 32'd200;
        @(posedge clk);
        #1;
        check(pc_plus_four_out === 32'd104,
              "branch address has priority over a simultaneous freeze");
        check(processor_unit.if_out_pc_plus_four === 32'd204,
              "cache wait does not discard a simultaneous EXE branch");
        check(processor_unit.id_in_instruction === 32'b0,
              "cache wait and branch collision flushes IF to ID");
        check(processor_unit.exe_in_wb_en === 1'b0,
              "cache wait and branch collision flushes ID to EXE");
        force processor_unit.exe_out_branch_taken = 1'b0;
        @(posedge clk);
        #1;
        check(processor_unit.if_out_pc_plus_four === 32'd204,
              "cache wait holds the processor branch target");
        @(negedge clk);
        force processor_unit.mem_out_stage_busy = 1'b0;
        @(posedge clk);
        #1;
        check(processor_unit.if_out_pc_plus_four === 32'd208,
              "processor PC resumes after the cache wait ends");
        branch_taken_in = 1'b0;
        @(posedge clk);
        #1;
